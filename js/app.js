...
var sportsURL = 'https://raw.githubusercontent.com/tooobeningcampground-exploration/main/data/sporting_store.geojson';
//menu for POI
...
function afterSearch(){
    ...
    (function loop(i, length) {
        ...
        var url = 'https://api.mapbox.com/geocoding/v5/mapbox.places/'+index[i]+'.json?access_token='+mapboxgl.accessToken
...
function loadGasLayer(){
    ...
    map.on('click', 'Gas', function (e) {
        ...
        if (e.features[0].properties.brands ==='null'){
            var popupContent = 'Class:'+gasCategory+'<br/>Sub-Class: '+gasSubCategory+'<br/>State:'+gasRegion+'<br/>'
        } else {
        var gasBrand = e.features[0].properties.brands;
        var popupContent = 'Brand:'+gasBrand+'<br/>Class:'+gasCategory+'<br/>Sub-Class: '+gasSubCategory+'<br/>State:'+gasRegion+'<br/>'
        }
...
function loadGroceryLayer(){
    ...
    map.on('click', 'Grocery', function (e) {
        ...
        if (e.features[0].properties.brands === 'null'){
            var popupContent = 'Class:'+groceryCategory+'<br/>Sub-Class: '+grocerySubCategory+'<br/>State:'+groceryRegion+'<br/>'
        } else {
            var groceryBrands = e.features[0].properties.brands;
            var popupContent = 'Brand: '+groceryBrands+'<br/>Class:'+groceryCategory+'<br/>Sub-Class: '+grocerySubCategory+'<br/>State:'+groceryRegion+'<br/>'
        }
        console.log(e);
...
function loadSportsLayer(){
    ...
    map.on('click', 'Sports', function (e) {
        ...
        if (e.features[0].properties.brands === 'null'){
            var popupContent = 'Class:'+sportsCategory+'<br/>Sub-Class: '+sportsSubCategory+'<br/>State:'+sportsRegion+'<br/>'
        } else{
            var sportsBrands = e.features[0].properties.brands;
            var popupContent = 'Brand: '+sportsBrands+'<br/>Class:'+sportsCategory+'<br/>Sub-Class: '+sportsSubCategory+'<br/>State:'+sportsRegion+'<br/>'
        }
...
function toggleMenu(SourceIdArray){
    var toggleableLayerImgs =['img/campsite-96.png','img/gas-96.png','img/grocery-96.png','img/sports-96.png']
    var toggleableLayerImgsAfter = ['img/campsite-96-al.png','img/gas-96-al.png','img/grocery-96-al.png','img/sports-96-al.png']
    for (var i = 0; i < toggleableLayerImgs.length; i++) {
        if (['Campsite','Pip-campsite','Pip-gas','Pip-grocery','Pip-sports'].indexOf(SourceIdArray[i]) >= 0){
...
function runDirection(ori,des){
    ...
    var routeURL = 'https://api.mapbox.com/directions/v5/mapbox/driving/'+startLng+'%2C'+startLat+'%3B'+endLng+'%2C'+endLat+'?alternatives=false&geometries=geojson&steps=false&access_token='+mapboxgl.accessToken;
    loadJSON(routeURL,
        function(jsonobject) {
            ...
            map.on('load', function () {              
                ...
                loadJSON(campsiteURL,function(jsonobject) {
                        // The API response is GeoJSON which has coordinates in lon/lat order
                        var coordinates = jsonobject.features;
                        var pip_array=[];

                        //iterate the features within the buffered zone
                        coordinates.forEach(function (feature) { //a feature is a point in {} format, with keys like "type","geometry","properties"
                            // Using Turf,                
                            // If a nearest library is found
                            if (typeof feature.geometry.coordinates[1] == 'number' && typeof feature.geometry.coordinates[1] == 'number' ) {
                        ...
                        result.innerHTML += '<p><b>, '+pipCampsiteNum+' campsites </b>in total. Enjoy it :)</p>';
                ...
                loadJSON(gasURL,function(jsonobject) {
                    // The API response is GeoJSON which has coordinates in lon/lat order
                    var coordinates = jsonobject.features;
                    var pip_array=[];
                    ...
                    const result = document.getElementById('result');
                    result.innerHTML += '<p>, <b>'+pipGasNum+'</b> gas station.</p>';
                ...
                loadJSON(groceryURL,function(jsonobject) {
                    // The API response is GeoJSON which has coordinates in lon/lat order
                    var coordinates = jsonobject.features;
                    var pip_array=[];
                    ...
                    const result = document.getElementById('result');
                    result.innerHTML += '<p>, <b>'+pipGroceryNum+'</b> grocery stores, and</p>';
                ...
                loadJSON(sportsURL,function(jsonobject) {
                // The API response is GeoJSON which has coordinates in lon/lat order
                var coordinates = jsonobject.features;
                var pip_array=[];
                ...
                const result = document.getElementById('result');
                result.innerHTML += '<p>In this area (100-miles buffered zone), you can find</p><p>, <b>'+pipSportsNum+'</b> sporting goods stores.</p>'
                //add a new source with the id 'pip-campsite'
        ...
        var SourcePipIdArray= ['Pip-campsite','Pip-gas','Pip-grocery','Pip-sports'];
...
function submitForm(event){
    ...
    (function loop(i, length) {
        ...
        var url = 'https://api.mapbox.com/geocoding/v5/mapbox.places/'+index[i]+'.json?proximity='+proximity+'&bbox='+searchBbox+'&type='+searchType+'&autocomplete=true&country=us&access_token='+mapboxgl.accessToken
        request.open("GET", url);
        request.responseType = 'json';
        request.onreadystatechange = function() {
            if(request.readyState === XMLHttpRequest.DONE && request.status === 200) {
